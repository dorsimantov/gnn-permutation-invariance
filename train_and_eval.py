...
parser.add_argument("-randomRatio", type=float, default=0.0)
parser.add_argument(
...
parser.add_argument("-normLayers", type=int, default=1)
parser.add_argument("-activation", type=str, default="tanh")
parser.add_argument("-learnRate", type=float, default=0.00065)
parser.add_argument("-learnRateGIN", type=float, default=0.00035)
parser.add_argument("-additionalRandomFeatures", type=int, default=1)
...
def graph_mixup(data1, data2, alpha=0.2):
    ...
    data_mix.x = mix * data1.x + (1 - mix) * data2.x

    edge_index = torch.cat((data1.edge_index, data2.edge_index), dim=1)
    data_mix.edge_index = torch.unique(edge_index, dim=1)
...
def drop_nodes(data, p=0.1):
    ...
    data.edge_index = node_map[data.edge_index[:, mask_edges]]
...
def permute_graph(graph):
    ...
    graph.edge_index = torch.tensor(
        [[node_mapping[s.item()] for s in row] for row in graph.edge_index],
        dtype=torch.long,
    )
    return graph
...
for i in range(SPLITS):
    ...
    permuted_train_loader = DataLoader(
        permuted_train_dataset, batch_size=BATCH, shuffle=True
    )
